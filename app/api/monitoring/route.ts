// نظام مراقبة شامل للاستيراد والأداء
...
function calculatePerformanceMetrics(): PerformanceMetrics {
  ...
  return {
    importSpeed: {
      ...
      currentRate: 0 // يمكن حسابها من المهام النشطة
    },
    apiLatency: {
      shopifyApi: 0, // يحتاج قياس فعلي
      invoiceCreation: 0, // يحتاج قياس فعلي
      database: 0 // يحتاج قياس فعلي
    },
    errorRates: {
      shopifyErrors: Math.round(errorRate * 0.3), // تقدير
      invoiceErrors: Math.round(errorRate * 0.5), // تقدير
      networkErrors: Math.round(errorRate * 0.2) // تقدير
    },
    resourceUsage: {
      cpuUsage: 0, // يحتاج قياس فعلي
      memoryUsage: (memoryUsage.heapUsed / memoryUsage.heapTotal) * 100,
      diskUsage: 0 // يحتاج قياس فعلي
    }
  }
}
...
function getErrorLog() {
  ...
  ).slice(0, 100) // آخر 100 خطأ
}

// GET - الحصول على معلومات المراقبة
  ...
  try {
    ...
    switch (action) {
      ...
      case 'health_check':
        ...
        // إرسال تنبيهات إذا كانت الحالة سيئة
