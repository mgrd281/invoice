...
export async function GET(req: Request) {
    try {
        console.log('⏰ Cron: Checking for Vorkasse reminders and cancellations...')
        ...
        for (const org of organizations) {
            const settings = org.vorkasseSettings || DEFAULT_SETTINGS

            if (!settings.enabled) {
                console.log(`Skipping org ${org.name} - Vorkasse automation disabled`)
                continue
            }
            ...
            for (const order of orders) {
                ...
                console.log(`Order ${order.name}: ${diffDays} days old. Gateway: ${gatewayStr}`)
                ...
                if (diffDays >= settings.cancellationDays) {
                    console.log(`🚫 Cancelling order ${order.name} (Age: ${diffDays} days)`)
                    try {
                      ...
                      <p>Ihre Bestellung ${order.name} wurde storniert, da wir keinen Zahlungseingang feststellen konnten.</p>
