...
export class ReminderEngine {
  ...
  public async processReminder(
    ...
    const subject = this.replaceTemplateVariables(template.subject, invoice, customer)
    const body = this.replaceTemplateVariables(template.body, invoice, customer)
    ...
    try {
      ...
      await this.sendEmail({
        ...
        subject: subject,
        body: body,
        attachments: this.settings.attachPdf ? [await this.generateInvoicePdf(invoice)] : []
      })
  ...
  public async sendManualReminder(
    ...
    return this.processReminder(queueItem, invoice, customer)
  }
  ...
  private async generateInvoicePdf(invoice: Invoice): Promise<Buffer> {
    // Mock PDF generation - integrate with your PDF service
    return Buffer.from(`PDF content for invoice ${invoice.number}`)
  }
}
