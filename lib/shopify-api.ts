...
export class ShopifyAPI {
  ...
  /**
   * Get all URL redirects from Shopify
   */
  async getRedirects(params: { limit?: number } = {}): Promise < any[] > {
      try {
        const searchParams = new URLSearchParams()
      ...
      const response = await this.makeRequest(`/redirects.json?${searchParams}`)
      const data = await response.json()
      return data.redirects || []
      } catch(error) {
        console.error('Error fetching redirects:', error)
  ...
  /**
   * Create a new URL redirect in Shopify
   */
  async createRedirect(path: string, target: string): Promise < any > {
      try {
        console.log(`🚀 Creating 301 redirect: ${path} -> ${target}`)
      const response = await this.makeRequest('/redirects.json', {
          method: 'POST',
          body: JSON.stringify({
            redirect: {
      ...
      const data = await response.json()
      console.log('✅ Redirect created successfully:', data.redirect?.id)
      return data.redirect
      } catch(error) {
        console.error('Error creating redirect:', error)
        throw error
  ...
  /**
   * Delete a redirect from Shopify
   */
  async deleteRedirect(redirectId: string | number): Promise < void> {
      try {
        console.log(`🗑️ Deleting redirect ${redirectId}...`)
      await this.makeRequest(`/redirects/${redirectId}.json`, {
          method: 'DELETE'
        })
      console.log('✅ Redirect deleted successfully')
      } catch(error) {
        console.error(`Error deleting redirect ${redirectId}:`, error)
        throw error
      }
    }

  /**
   * Helper to get all entity handles/paths for URL matching
   */
  async getAllStorefrontPaths(): Promise < { products: string[], collections: string[], pages: string[] } > {
      try {
        const [products, collections, pages] = await Promise.all([
  ...
  async getPages(params: { limit?: number } = {}): Promise < any[] > {
      ...
      const response = await this.makeRequest(`/pages.json?${searchParams}`)
      const data = await response.json()
      return data.pages || []
      } catch(error) {
        console.error('Error fetching pages:', error)
